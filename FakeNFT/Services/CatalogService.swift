        ...
        networkClient.send(request: request, type: [CatalogNetworkModel].self) { result in
            ...
            case .success(let model):
                let model = model.map { collection in
                    CatalogCellModel(imageCollection: collection.cover, collectionName: collection.name, nftCount: collection.nfts.count,author: collection.author, description: collection.description)
                }
